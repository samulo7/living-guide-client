...
class Service {
	...
	async search(data = {}) {
		...
		const pois = Array.isArray(originalResult.pois) ? originalResult.pois : [];
		let _data = pois.map((item) => {
			const business = (item && typeof item.business === "object") ? item.business : {};
			const children = Array.isArray(item && item.children) ? item.children : [];
			const locationValue = String(item && item.location || "");
			const parsedLocation = locationValue != "" ? libs.common.getLocation(locationValue, "lng,lat", "object") : {
				lat: NaN,
				lng: NaN
			};
			return {
				...
				children: children.length > 0 ? children.map((item2) => {
					const childLocationValue = String(item2 && item2.location || "");
					return {
						parent_id: item.id,
						id: item2.id,
						title: item2.name,
						...
						location: childLocationValue != "" ? libs.common.getLocation(childLocationValue, "lng,lat", "object") : {
							lat: NaN,
							lng: NaN
						}
					}
