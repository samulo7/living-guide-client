...
console.log("uni-map-co loaded, version:", Date.now());
...
const LocalMapConfig = {
	...
	"key": {
		"qqmap": "", // 腾讯地图key
		"amap": "56ab408890a598cb031e5ca5a9f002d7", // 高德地图key
	}
}

function safeMapConfig(config) {
	if (!config || typeof config !== 'object') {
		return LocalMapConfig;
	}
	const key = (config.key && typeof config.key === 'object') ? config.key : {};
	return {
		default: config.default || LocalMapConfig.default || "qqmap",
		key: {
			qqmap: key.qqmap || "",
			amap: key.amap || ""
		}
...
function pickProvider(inputProvider, mapConfig) {
	let provider = inputProvider || mapConfig.default || "qqmap";
	// H5 chooseLocation often sends qqmap; fallback to amap when qq key is absent.
	if (provider === "qqmap" && !mapConfig.key.qqmap && mapConfig.key.amap) {
		provider = "amap";
	}
	return provider;
}
...
module.exports = {
	_before: function() {
		...
		UniMapConfig = safeMapConfig(UniMapConfig);
		...
		provider = pickProvider(provider, UniMapConfig);
	...
	_after: function(err, res) {
		if (err) {
			...
			const errMsg = String((err && (err.message || err.msg || err.errMsg)) || "").trim();
			const detail = safeErrorDetail(err);
			return {
				errCode: "SYSTEM_ERROR",
				errMsg: `[uni-map-co v20260227.2]: ${errMsg || detail || "unknown system error"}`
			};
	...
	async request(data = {}) {
		...
		const configCenter = require("uni-config-center");
		// 读取配置中心地图配置
		const UniMapConfig = safeMapConfig(configCenter({ pluginId: 'uni-map' }).requireFile('config.js'));
		// 获取uniMap实例
		const provider = pickProvider("", UniMapConfig);
		const uniMap = new UniMap({
			provider: provider, // 指定使用哪家地图供应商
			key: UniMapConfig.key[provider], // 你的地图密钥
